...
import { Mail, MapPin, ArrowUpRight, Github, Linkedin, Twitter, CheckCircle, Send } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-14">
          <motion.div className="space-y-6" {...fadeInUp(0.15)}>
            <div className="group flex items-center gap-4 p-5 rounded-2xl border border-border bg-muted/20 hover:bg-muted/30 transition-all duration-300">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm md:text-base font-medium truncate text-foreground">{profile.email}</p>
              </div>
              <button
              ...
              >
                {copied ? <CheckCircle size={18} className="text-emerald-500" /> : <ArrowUpRight size={18} />}
              </button>
            </div>
          ...
          >
            ...
            <div className="space-y-2">
              <label htmlFor="message" className="text-xs font-bold text-muted-foreground ml-1 uppercase tracking-tighter">
                {contact.message}
              </label>
              <textarea
                id="message"
            ...
            <AnimatePresence mode="wait">
                {status === "success" ? (
                    ...
                    >
                        {language === 'en' ? 'There was an error. Please try again.' : 'Hubo un error. Por favor, reintenta.'}
                    </motion.div>
                ) : (
                    <Button
                        key="submit"
                    ...
                    >
                        {status === "loading" ? (
                            ...
                            <>
                                {contact.send} <Send className="ml-3 w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                            </>
                        )}
                    </Button>
                )}
...
const FormField: React.FC<{
  ...
  <div className="space-y-2">
    <label htmlFor={id} className="text-xs font-bold text-muted-foreground ml-1 uppercase tracking-tighter">
      {label}
    </label>
    <input
      id={id}
