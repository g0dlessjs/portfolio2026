  ...
  return (
    <section id="projects" className="py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="container px-4 md:px-6 mx-auto relative z-10 max-w-7xl">
        {/* Heading */}
        <motion.div
          className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 md:mb-12 gap-4"
          {...fadeInUp()}
        >
          <div>
          ...
          <Button
            onClick={onViewAll}
            className="rounded-full px-5 h-10 text-sm shrink-0 mt-2 sm:mt-0"
            variant="outline"
          >
    ...
    <div className="group relative rounded-4xl overflow-hidden cursor-pointer bg-muted/20 h-full transition-all duration-700 hover:shadow-[0_0_50px_-12px_rgba(var(--primary-rgb),0.3)] ring-1 ring-white/10 hover:ring-primary/40">
      ...
      <div className="absolute top-6 left-6 flex gap-2">
        {project.tags.slice(0, 2).map((tag: string) => (
          <span
            key={tag}
            className="text-[9px] font-bold tracking-widest text-white bg-black/40 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 uppercase"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="absolute bottom-0 left-0 p-8 md:p-10 w-full translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
        ...
        <p className="text-muted-foreground text-base md:text-lg line-clamp-2 max-w-xl mb-6 font-light">
          {project.description}
        </p>
      ...
      <div className="absolute bottom-0 left-0 p-6 md:p-8 w-full transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
        <div className="mb-3 flex flex-wrap gap-2">
          {project.tags.slice(0, 1).map((tag: string) => (
            <span
              key={tag}
              className="text-[8px] font-bold text-primary tracking-widest uppercase bg-primary/10 border border-primary/20 px-2 py-0.5 rounded-full backdrop-blur-md"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex items-center justify-between gap-4">
          <h3 className="text-xl md:text-2xl font-display font-bold text-foreground leading-tight group-hover:text-primary transition-colors">
            {project.title}
          </h3>
    ...
    <section className="pt-12 md:pt-16 pb-24 container px-4 md:px-6 mx-auto max-w-7xl min-h-screen">
      <nav className="mb-8 md:mb-12">
        <Button
          variant="ghost"
          onClick={onBack}
          className="pl-0 hover:pl-2 text-muted-foreground hover:text-foreground transition-all gap-2 group"
        >
      ...
      <motion.div className="mb-12 md:mb-16" {...fadeInUp()}>
        <h1 className="text-4xl sm:text-5xl md:text-7xl font-display font-bold mb-6 tracking-tight">
          {t.projects.archiveTitle.split(' ')[0]}{' '}
          <span className="text-primary">
            {t.projects.archiveTitle.split(' ').slice(1).join(' ')}
          </span>
        </h1>
...
export const ProjectDetail: React.FC<{ id: number; onBack: () => void }> = ({
  id,
  onBack,
}) => {
  const { t } = useLanguage();
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap gap-2 mb-2">
              {project.tags.map((tag: string) => (
                <span
                  key={tag}
                  className="text-[10px] font-bold text-primary tracking-widest uppercase border border-primary/20 bg-primary/5 px-2.5 py-1 rounded-full"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-20">
          <motion.div className="lg:col-span-4 space-y-10" {...fadeInUp(0.1)}>
            <div className="space-y-8 sticky top-24">
              <DetailItem
                label="Cliente"
                value={project.client || 'Sector Tech'}
              ...
              <DetailItem
                label="Rol"
                value={project.role || 'Lead Architect'}
                icon={<Layers size={16} />}
              />
              ...
              <div className="pt-6 border-t border-border flex gap-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:opacity-90 active:scale-95 transition-all"
                >
                  Live Demo <Globe size={16} aria-hidden="true" />
                </a>
                <a
                  href="#"
                  aria-label="Ver el repositorio de GitHub de este proyecto"
                  className="w-12 h-12 flex items-center justify-center rounded-xl bg-muted border border-border text-muted-foreground hover:text-foreground transition-all"
                >
          ...
          <motion.div
            className="lg:col-span-8 space-y-16 md:space-y-24"
            {...fadeInUp(0.2)}
          >
            <ProjectContentSection
              ...
              content={
                project.challenge ||
                'Desarrollar una infraestructura robusta que soporte el crecimiento exponencial del tráfico web.'
              }
              number="01"
            />
            <ProjectContentSection
              ...
              content={
                project.solution ||
                'Implementación de una arquitectura basada en microservicios comunicados vía gRPC.'
              }
              number="02"
            />
            <ProjectContentSection
              ...
              content={
                project.result ||
                'Aumento de la eficiencia en el procesamiento en un 40%.'
              }
              number="03"
            />
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-base font-bold font-display uppercase tracking-widest text-muted-foreground">
...
const ProjectContentSection: React.FC<{
  title: string;
  content: string;
  number: string;
}> = ({ title, content, number }) => (
  <div className="space-y-6 relative">
    <div className="absolute -left-8 md:-left-14 top-0 text-5xl md:text-7xl font-display font-bold text-primary/5 select-none">
      {number}
    </div>
