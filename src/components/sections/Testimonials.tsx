...
export const Testimonials: React.FC = () => {
  ...
  const getCardWidth = () => {
    ...
    return window.innerWidth < 768 ? window.innerWidth * 0.85 : 350;
  };

  const [cardWidth, setCardWidth] = useState(getCardWidth());
  const gap = typeof window === 'undefined' ? 24 : (window.innerWidth < 768 ? 16 : 24);
  ...
  // Calculate total carousel width for centering
  ...
  return (
    <section id="testimonials" className="py-16 md:py-24 relative overflow-hidden bg-background">
      ...
      <div className="relative" ref={containerRef}>
        {/* Desktop navigation arrows - positioned OUTSIDE the carousel track */}
        <div className="hidden md:flex absolute top-1/2 -translate-y-1/2 left-0 right-0 justify-between px-4 md:px-8 pointer-events-none z-20">
          <button
            onClick={prev}
        ...
        {/* Carousel track - centered horizontally */}
        <div className="overflow-hidden">
          ...
          >
            {testimonials.map((test: any, i: number) => (
              ...
              >
                <div className={`bg-card/40 backdrop-blur-xl border ${i === index ? 'border-primary/30 shadow-2xl shadow-primary/5' : 'border-border/50'} rounded-3xl md:rounded-4xl p-5 md:p-10 relative overflow-hidden group h-[340px] md:h-[400px] flex flex-col justify-between transition-all duration-500`}>
                  {/* Quote icon */}
                  ...
                  <div className="relative z-10 flex-1 flex items-center">
                    <p className={`${i === index ? 'text-foreground' : 'text-muted-foreground/60'} text-sm md:text-xl font-light leading-relaxed italic`}>
                      "{test.content}"
                    </p>
                  </div>

                  {/* Author info */}
                  <div className="flex items-center gap-3 md:gap-5 relative z-10 pt-4 md:pt-6 border-t border-white/10">
