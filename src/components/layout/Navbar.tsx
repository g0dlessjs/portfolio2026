...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const sections = [
    { id: "home", icon: Home, label: language === 'es' ? 'Inicio' : 'Home', show: portfolioConfig.sections.showHero },
    { id: "about", icon: User, label: language === 'es' ? 'Perfil' : 'About', show: portfolioConfig.sections.showAbout },
    { id: "services", icon: Wrench, label: language === 'es' ? 'Servicios' : 'Services', show: portfolioConfig.sections.showServices },
    { id: "experience", icon: CalendarDays, label: language === 'es' ? 'Experiencia' : 'Experience', show: portfolioConfig.sections.showExperience },
    { id: "projects", icon: Briefcase, label: language === 'es' ? 'Trabajo' : 'Work', show: portfolioConfig.sections.showProjects },
    { id: "testimonials", icon: Quote, label: language === 'es' ? 'Opiniones' : 'Testimonials', show: portfolioConfig.sections.showTestimonials },
    { id: "contact", icon: Mail, label: language === 'es' ? 'Contacto' : 'Contact', show: portfolioConfig.sections.showContact },
  ].filter(s => s.show);
      ...
      <header className="fixed top-0 inset-x-0 z-50">
        <div className="bg-background/60 dark:bg-background/40 backdrop-blur-xl border-b border-border/40">
          <div className="container px-4 md:px-6 mx-auto flex items-center justify-between h-20">
            ...
            >
              {t.profile.brandName.split('.')[0]}<span className="text-secondary">{t.profile.brandName.includes('.') ? '.' : ''}</span>{t.profile.brandName.split('.')[1] || ''}
            </button>
            ...
            <nav className="hidden lg:flex items-center gap-1" aria-label="Navegación principal">
              ...
              <div className="relative">
                ...
                >
                  <Globe size={22} />
                  <span className="absolute -top-1 -right-1 text-[8px] font-bold bg-primary text-primary-foreground px-1 rounded-sm uppercase">{language}</span>
                </button>
                  ...
                  <div className="absolute top-full right-0 mt-3 w-32 bg-popover/90 backdrop-blur-xl border border-border rounded-2xl shadow-2xl overflow-hidden p-1">
                    {(["es", "en"] as const).map((lang) => (
                      ...
                      >
                        ...
                        {language === lang && <div className="w-1.5 h-1.5 rounded-full bg-current" />}
                      </button>
                    ))}
                  </div>
                )}
            ...
            <nav className="flex flex-col gap-1">
              ...
              <div className="grid grid-cols-2 gap-2">
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 p-4 rounded-2xl bg-muted text-foreground font-bold text-xs"
                  >
                    {theme === 'dark' ? <Sun size={18} className="text-amber-400" /> : <Moon size={18} className="text-zinc-800" />}
