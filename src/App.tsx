...
// Lazy Loaded Components (Mejora: Reduce el JS inicial, Code Splitting)
const About = lazy(() =>
...
const AllProjects = lazy(() =>
  import('./components/sections/Projects').then(m => ({ default: m.AllProjects }))
);
const ProjectDetail = lazy(() =>
  import('./components/sections/Projects').then(m => ({ default: m.ProjectDetail }))
);
...
const AppContent: React.FC = () => {
  ...
  const getPageTitle = () => {
    if (currentView === 'all-projects')
      return language === 'es' ? 'Archivo de Proyectos' : 'Project Archive';
    return 'Home';
  };
  ...
  return (
    <main className="min-h-screen bg-background text-foreground transition-colors duration-500 selection:bg-primary/30 selection:text-foreground pt-16">
